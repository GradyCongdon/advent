const d1 = `[1518-03-19 00:02] Guard #647 begins shift
...
[1518-04-12 00:23] falls asleep`.split('\n');

function parse(line) {
  const [_, dateString, statusString] = /\[(.*?)\] (.*)/.exec(line);
  const date = new Date(dateString);
  let status;
  if (statusString.match('Guard')) {
    const [_all, id] = /Guard #(\d+) begins/.exec(statusString);
    status = +id;
  } else {
    status = statusString.match('wakes') ? 'WAKE' : 'SLEEP';
  }
  return {
    date,
    status,
...
const data = d1;
let actions = data.reduce((acc, line) => {
  acc.push(parse(line));
  return acc;
}, []);
...
let time = actions[0].date;
const maxTime = actions[actions.length - 1].date;
let actionIndex = 0;
while (time < maxTime) {
  const { date, status } = actions[actionIndex];
    ...
    switch (status) {
      ...
      default:
        ...
        if (!guards[status]) {
          ...
          guards[status].asleep = false;
        }
        guardId = status;
    }
  }
  if (guard && guard.asleep) {
    guard.times.push(time);
    guard.minutes.push(time.getMinutes());
  }
  time = new Date(time.getTime() + 60000); // add minute
}


let guardMinutes = guards
  .filter(g => g)
  .map(g => {
    return {
      id: g.id,
      counts: mc(g),
      minutes: maxMinutes(g),
    };
  });

function mc(g) {
  const minuteCounts = g.minutes.reduce((counts, minute) => {
...
function maxMinutes(g) {
  ...
  const mostMinute = Object.keys(minuteCounts).reduce((max, key) => {
    const v = minuteCounts[key];
    if (v > max.v) {
      max = {
        k: key,
        v: v,
...
guardMinutes = guardMinutes.sort((a,b) => a.minutes.v - b.minutes.v);

//console.log(guardMinutes);

console.log(guardMinutes.pop());
